...
function monta_links ( $texto , $vetorDeLexicos , $vetorDeCenarios ) 
{
	...
	while($indice < count( $vetorAuxLexicos ) )
	{
		...
		$regex = "/(\s|\b)(" . $nomeLexico . ")(\s|\b)/i";
		$texto = preg_replace( $regex,"$1<a title=\"Léxico\" href=\"main.php?t=l&id=".$vetorAuxLexicos[$indice]->id_lexico."\">".$vetorAuxLexicos[$indice]->nome."</a>$3", $texto );
		$indice++;
	}
	...
	while($indice < count( $vetorAuxCenarios) )
	{
		...
		$regex = "/(\s|\b)(" . $tituloCenario . ")(\s|\b)/i";
		$texto = preg_replace( $regex,"$1<a title=\"Cenário\" href=\"main.php?t=c&id=".$vetorAuxCenarios[$indice]->id_cenario."\"><span style=\"font-variant: small-caps\">".$vetorAuxCenarios[$indice]->titulo."</span></a>$3", $texto );
		$indice++;
	}
